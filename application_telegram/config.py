...
database_engine = create_engine(
    f"postgresql+psycopg2://{DATABASE_LOGIN}:{DATABASE_PASSWORD}@{SERVER_IP}:{DATABASE_PORT}/{DATABASE_NAME}",
)
database_engine_async = create_async_engine(
    f"postgresql+asyncpg://{DATABASE_LOGIN}:{DATABASE_PASSWORD}@{SERVER_IP}:{DATABASE_PORT}/{DATABASE_NAME}",
)
